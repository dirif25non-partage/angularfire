...
Pour déployer sur Firebase, il faut :
   ...
   * entrer :code:`firebase login` ( avec éventuellement :code:` –no-localhost`)
...
Cependant, les fichiers produits par le build sont placés dans le répertoire browser qui est lui même dans le répertoire outpout renseigné dans angular.json ...

Il faut mettre en cohérence ces paramétrages, à la main.

Ensuite pour que le *routage* fonctionne, il faut ajouter des lignes à firebase.json :
